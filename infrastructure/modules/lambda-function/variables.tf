...
variable "enable_param_store_secrets_extension" {
  type        = bool
  default     = false
  description = "Enable the Lambda extension (layer) to fetch and cache SSM parameters & Secrets Manager secrets."
}

variable "timeout" {
  type        = number
  description = "Maximum Lambda run duration."
}
